...
used for creating the required API. If the response is retrieved successfully, it will be converted to a 
CreditUiModel which will be assigned to the exposed LiveData, so the Activity will be notified and use the 
uiModel to update the donut view.
...
network responses from real JSON data have been used.
